---
name: tofugraph-label
description: 의미 이름표 실습 — 서브에이전트를 여러 개 스폰해 창고의 관계 전체에 의미 이름표(belongs_to·part_of 등 13종)를 붙입니다
---

# $tofugraph-label (Codex용 래퍼 — 정본 절차는 플러그인의 commands/label.md)

이 스킬은 Claude Code 명령 `/tofugraph:label` 의 Codex 판입니다. 절차 본문은 한 곳(정본)만 둡니다:

1. **플러그인 루트 계산**: 이 SKILL.md 가 있는 폴더에서 **세 단계 위**가 플러그인 루트다.
   ```bash
   PLUGIN_ROOT="$(cd "$(dirname '<이 SKILL.md 의 절대경로>')/../../.." && pwd)"
   ```
   (`$PLUGIN_ROOT/commands/label.md` 와 `$PLUGIN_ROOT/scripts/` 가 실제로 존재하는지 `ls` 로 1회 확인 후 진행.)
2. **정본 절차 실행**: `$PLUGIN_ROOT/commands/label.md` 를 Read 하고 그 절차를 그대로 따른다. 단 Codex 환경 번역 규칙:
